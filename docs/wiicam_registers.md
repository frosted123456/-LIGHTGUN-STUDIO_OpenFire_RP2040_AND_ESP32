...
| A sun streak 25–31 px wide reported at `size` 2, the same byte as the LEDs, sat in a sensor slot through a LOWER 139 → 81; the streak stayed, the LEDs went. Consistent with the hypothesis. The loop now reads the `size` byte: a stray no bigger than the corners in it is not MAXSIZE's to cut, and the dwell holds (`uncut=` on `~camloop?`). | `blobs003013303` (Pi, sun) | Measured, one capture |
